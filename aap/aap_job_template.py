...
class AapJobTemplate(AapBase):
    ...
    def __init__(self,
        ...
        log.debug(f"Initializing {self.__class__}: [{name}, {id_}, {url}, {description}]")
        AapBase.__init__(self, id_=id_, url=url)
        self.name = name
        self.description = description

    def __eq__(self, other):
        if isinstance(other, AapJobTemplate):
    ...
    def from_aap(cls, data: dict):
        ...
        try:
            ...
            raise ValueError("Failed to get a required job template parameter") from e
